...
public class Mapa {
    ...
    public boolean isParede(int l, int c) { return grid[l][c] == '#'; }

    public boolean temDinossauro(int l, int c) { return getDinossauro(l, c) != null; }
    
    public Dinossauro getDinossauro(int l, int c) {
        for (Dinossauro d : dinossauros) {
            if (d != null && d.getLinha() == l && d.getColuna() == c && d.isVivo()) return d;
        }
        return null;
    }
    
    public void removerDinossauro(Dinossauro d) { dinossauros.remove(d); }

    public boolean temCaixa(int l, int c) { return getCaixa(l, c) != null; }
    
    public Caixa getCaixa(int l, int c) {
        for (Caixa cx : caixas) {
            if (cx != null && cx.getLinha() == l && cx.getColuna() == c) return cx;
        }
        return null;
    }
    
    public void removerCaixa(Caixa cx) { caixas.remove(cx); }
    ...
    public void moverInimigos(Jogador jogador) {
        for (Dinossauro d : dinossauros) {
            ...
            if (d == null || d instanceof TRex || !d.isVivo()) continue;
            
            int movs = (d instanceof Velociraptor) ? 2 : 1;
            for (int i = 0; i < movs; i++) {
                ...
                if (posicaoValida(nl, nc) && !isParede(nl, nc) && !temDinossauro(nl, nc)) {
                    d.setPosicao(nl, nc);
                    if (d.getLinha() == jogador.getLinha() && d.getColuna() == jogador.getColuna()) {
                        System.out.println("\n*** UM INIMIGO SURGE DAS SOMBRAS! O " + d.getNome() + " ataca! ***");
                        int esquiva = rand.nextInt(3) + 1;
                        if (esquiva > jogador.getPercepcao()) {
                            jogador.sofrerDano(1);
                            System.out.println("Voce nao reagiu a tempo e sofreu 1 de dano!");
                        } else {
                            System.out.println("Voce estava preparado e desviou do ataque surpresa!");
                        }
                        break;
                    }
                }
