...
public class Mapa implements Serializable {
    ...
    public boolean isParede(int l, int c) { return grid[l][c].isParede(); }

    public boolean temDinossauro(int l, int c) { return grid[l][c].getDinossauro() != null; }
    
    public Dinossauro getDinossauro(int l, int c) { return grid[l][c].getDinossauro(); }
    
    public void removerDinossauro(Dinossauro d) {
        grid[d.getLinha()][d.getColuna()].setDinossauro(null);
        dinossauros.remove(d);
    }

    public boolean temCaixa(int l, int c) { return grid[l][c].getCaixa() != null; }
    
    public Caixa getCaixa(int l, int c) { return grid[l][c].getCaixa(); }
        
    public void removerCaixa(Caixa cx) {
        grid[cx.getLinha()][cx.getColuna()].setCaixa(null);
        caixas.remove(cx);
    }

    public boolean todosDinossaurosDerrotados() { return dinossauros.isEmpty(); }

    // Método corrigido e limpo: cuida puramente da movimentação das entidades
    public void moverInimigos(Jogador jogador) {
        for (Dinossauro d : dinossauros) {
            if(d == null || !d.isVivo()) continue;
